...
if (site_sar_available) {
  ...
  if (nrow(site_sar_data) > 0) {
    ...
    kable(sar_tbl, format = "latex", booktabs = TRUE, align = "lrcccclc",
          caption = paste0(report_name, " — Most Recent SAR Results")) |>
...
if (!is.null(ot_data)) {
  ...
  if (nrow(spec_ot) > 0) {
    cat("The following charts show historical O/E ratios across SAR periods for\n")
    cat("complications with available trend data. Red dots indicate periods where\n")
    cat("the site was flagged as a high outlier.\n\n")
...
*Report generated `r format(Sys.time(), "%Y-%m-%d %H:%M")` | NSQIP CUSUM Monitoring System v1.2*
\
